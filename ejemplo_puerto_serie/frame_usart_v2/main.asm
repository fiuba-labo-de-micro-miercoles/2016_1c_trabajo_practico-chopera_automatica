;
; frame_usart_test.asm
;
; Created: 29/05/2016 07:30:56 p.m.
; Author : Tobias
;
 ...
 *  Author: Zoso
	...
	Pense en un pequeño test que si el primer byte del 
...
RESET:	ldi 	r16,LOW(RAMEND)	
		...
		ldi	t0,0xFF	;Cambios en el puerto C
		out	DDRC,t0
		ldi	t0,0xFF
		out	PORTC,t0
		...
		;rcall	TEST_TX 
		;comento TEST_TX porque habilita la interrupcion para enviar UDRIE0
		;se puede descomentar al programarlo
...
USART_init:
		...
		ldi		t0,(1<<RXCIE0)|(1<<RXEN0)|(1<<TXEN0)|(0<<UDRIE0)|(0<<TXCIE0)
		output	UCSR0B,t0
...
;-->bytes_recibidos debe estar en 0 antes de empezar
;-->ptr_rx_L y ptr_rx_H deben apuntar a RX_BUF (Vale la pena validar? si solo valido contra RX_BUF porque no lo seteo de una?)
;
...
ISR_RX_USART_STREAM:
		...
		input		t1,UCSR0B
		cbr		t1,(1<<RXCIE0);Pone en 0 RXCIE0 (desabilita las interrupciones)
		output	UCSR0B,t1
		...
		breq		ISR_RX_ERR_END	;si no esta vacio, cancelo recepcion
...
ISR_RX_BYTE:
		...
		st		X+,t0
		inc		t2
		cpi		XH,high(RX_BUF+BUFFER_SIZE);ojo que hay que comparar primero la parte alta y luego la baja
		brsh		ISR_RX_END
...
ISR_RX_ERR_END:
		...
		;si no hago los pop vuelve a cualquier lado
...
FIN_TRANSMISION:		; si no hay nada que enviar,
		input		t0,UCSR0B
		cbr		t0,(1<<UDRIE0);borro el bit de UDRIE0 para que no vuelva a interrumpir
		output	UCSR0B,t0
...
TEST_TX:
		...
		movw		XH:XL,ptr_tx_H:ptr_tx_L ;Modificado
